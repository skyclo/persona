...
export default function Globe() {
    ...
    const [loadedInfo, setLoadedInfo] = useState<{loaded: boolean; width?: number; height?: number}>({ loaded: false });
    ...
    useEffect(() => { isPausedRef.current = isPaused; autoRotateRef.current = !isPaused; }, [isPaused]);

    useEffect(() => {
        const { Scene, PerspectiveCamera, WebGLRenderer, Mesh, SphereGeometry, MeshStandardMaterial, Color, Points, PointsMaterial, BufferGeometry, Float32BufferAttribute, AdditiveBlending, AmbientLight, DirectionalLight } = THREE as any;
    ...
    const globeMat = new MeshStandardMaterial({ color: 0x042a6b, metalness: 0.1, roughness: 0.78, transparent: true, opacity: 0.9, alphaTest: 0.02 });
    ...
    const atmosphereGeo = new SphereGeometry(1.7, 64, 64);
        const atmosphereMat = new THREE.ShaderMaterial({
            ...
            depthWrite: false,
            uniforms: {
                // brighten inner color so globe center is less dark
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // center darker, edge brighter; add a small base from innerColor so center isn't too dark
                    vec3 base = innerColor * 0.18;
                    vec3 col = mix(innerColor, outerColor, fresnel * intensity) + base;
                    // sun lighting on atmosphere: enhance color on sun-facing normals
        ...
        const karmanLineGeo = new SphereGeometry(1.72, 64, 64);
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // stronger sun-dependent rim glow
                    float sunN = max(dot(normalize(vNormal), normalize(sunDir)), 0.0);
    ...
    scene.add(karmanLine);
        ...
        const starsMat = new PointsMaterial({ color: 0xffffff, size: 0.15, sizeAttenuation: true, depthWrite: false });
    ...
    const textureLoader = new THREE.TextureLoader();
        textureLoader.load(
            ...
            (tex) => {
                ...
                try { tex.minFilter = (THREE as any).LinearMipmapLinearFilter || tex.minFilter; } catch (e) { }
                try { tex.magFilter = (THREE as any).LinearFilter || tex.magFilter; } catch (e) { }
                try {
                    let maxAniso = 1;
                    const caps = (renderer as any).capabilities;
                    if (caps && typeof caps.getMaxAnisotropy === 'function') maxAniso = caps.getMaxAnisotropy();
                    else if ((renderer as any).capabilities && (renderer as any).capabilities.maxAnisotropy) maxAniso = (renderer as any).capabilities.maxAnisotropy;
                ...
                if (seemsEmpty) {
                    ...
                    console.warn('Primary world_map.png appears empty; attempting fallback world_map_modified.png');
                    textureLoader.load('/images/world_map_modified.png', (alt) => {
                        try { (alt as any).encoding = (THREE as any).sRGBEncoding; } catch (e) { }
                        ...
                        try {
                            ...
                            if (ctx2) {
                                ...
                                for (let i2 = 0; i2 < data2.length; i2 += 4) {
                                    if (data2[i2 + 3] === 0) {
                                        data2[i2] = 4; data2[i2 + 1] = 42; data2[i2 + 2] = 107; data2[i2 + 3] = 255;
                                    }
                                }
                                ctx2.putImageData(id2, 0, 0);
                                const newTex2 = new THREE.CanvasTexture(canvas2);
                                try { (newTex2 as any).encoding = (THREE as any).sRGBEncoding; } catch (e) {}
                                newTex2.flipY = true;
                                try { (newTex2 as any).format = (THREE as any).RGBAFormat; } catch (e) {}
                                try { newTex2.anisotropy = (renderer as any).capabilities?.getMaxAnisotropy ? (renderer as any).capabilities.getMaxAnisotropy() : 1; } catch (e) {}
                        ...
                        alt.flipY = true;
                        try { (alt as any).format = (THREE as any).RGBAFormat; } catch (e) { }
                        ...
                        setLoadedInfo({ loaded: true, width: imgAlt?.width || 0, height: imgAlt?.height || 0 });
                    });
                } else {
                    ...
                    try {
                        ...
                        if (ctx) {
                            ...
                            for (let i = 0; i < data.length; i += 4) {
                                if (data[i + 3] === 0) {
                                    data[i] = 4; data[i + 1] = 42; data[i + 2] = 107; data[i + 3] = 255;
                                }
                            }
                            ctx.putImageData(id, 0, 0);
                            const newTex = new THREE.CanvasTexture(canvas);
                            try { (newTex as any).encoding = (THREE as any).sRGBEncoding; } catch (e) {}
                            newTex.flipY = true;
                            try { (newTex as any).format = (THREE as any).RGBAFormat; } catch (e) {}
                            try { newTex.anisotropy = (renderer as any).capabilities?.getMaxAnisotropy ? (renderer as any).capabilities.getMaxAnisotropy() : 1; } catch (e) {}
                            ...
                            tex.flipY = true;
                            try { (tex as any).format = (THREE as any).RGBAFormat; } catch (e) { }
                            ...
                            setLoadedInfo({ loaded: true, width: im?.width || 0, height: im?.height || 0 });
                        }
                    } catch (e) {
                        tex.flipY = true;
                        try { (tex as any).format = (THREE as any).RGBAFormat; } catch (e) { }
                        ...
                        setLoadedInfo({ loaded: true, width: im2?.width || 0, height: im2?.height || 0 });
                    }
                }
            },
            undefined,
    ...
    try {
        ...
        try { sunHelper.visible = false; } catch (e) {}
        scene.add(sunHelper);
    ...
    let sunInterval: ReturnType<typeof setInterval> | null = setInterval(() => sunUpdateRef.current && sunUpdateRef.current(), 60 * 1000);
        ...
        const onPointerUp = () => {
            ...
            try { sunUpdateRef.current && sunUpdateRef.current(); } catch (e) { /* ignore */ }
        };
    ...
    const animate = () => {
        ...
        try {
            ...
            if (dt >= 500) { // update display every 0.5s
                const fps = (fpsRef.current.frames * 1000) / dt;
                // smooth a bit
                fpsRef.current.fps = fpsRef.current.fps ? (fpsRef.current.fps * 0.6 + fps * 0.4) : fps;
    ...
    return (
        <div className="w-full h-full three-canvas relative">
            <div ref={mountRef} className="absolute inset-0" />
            <div className="rotation-slider fixed top-16 left-4 group z-50">
                <div className="slider-handle w-8 h-8 bg-gray-900 rounded-sm flex items-center justify-center text-xs text-white opacity-90 ring-1 ring-gray-600">▼</div>
                <div className="hidden group-hover:flex items-center ml-2 bg-gray-900/60 p-2 rounded">
                    <button
                        ...
                        className="mr-2 flex items-center justify-center w-8 h-8 bg-gray-800 text-white rounded"
                    >
                        {/* pause vs play icon */}
                        {!isPaused ? (
                    ...
                    >
                        {/* diameter icon: circle with a horizontal line through center */}
            ...
            {showDebug && (
                <div className="absolute top-48 left-0 z-60 bg-black/80 text-white p-3 rounded text-sm max-w-xs">
                    <div className="font-bold mb-1">Globe debug</div>
                    ...
                    {loadedInfo.loaded && (
                        <div className="mt-2">
                            <div>Image: {loadedInfo.width || 0} x {loadedInfo.height || 0}</div>
                        </div>
                    )}
                </div>
            )}
